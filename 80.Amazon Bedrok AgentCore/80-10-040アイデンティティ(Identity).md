...
Identityを介して、ユーザーの権限で外部API（Gateway経由）を呼び出す実装例です。

```
# --- 1. 必要パッケージのインポート ---
# エージェント構築とBedrockモデルの連携
from strands.agents import Agent
from strands.llms import BedrockChat # またはChatBedrockConverse
# Identity関連の定義
from bedrock_agentcore.identity import InboundJwtAuthorizer

# --- 2. Inbound（入り口）の認証設定 ---
# 外部IdP（Cognitoなど）からのJWTトークンを検証する実体
authorizer = InboundJwtAuthorizer(
    authorizer_id="auth-id-from-aws-console", # AWS上で作成済みのID
)

# --- 3. エージェントの構築 ---
# ここでauthorizerを紐付けることで、このエージェントは「認証済みユーザー」として振る舞う
my_agent = Agent(
    name="SecureIdentityAgent",
...
# --- 4. 実行時のポイント（Outbound） ---
# 実行時、ユーザーから渡されたJWTトークンをヘッダーに含める
# エージェントはこのトークンを元に、Gateway側で管理されている「ユーザーAのSlackキー」
# を自動的に使ってメッセージを送信します。
user_jwt = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."

response = my_agent.run(
    "私のSlackに今日の売上報告を投稿して。",
