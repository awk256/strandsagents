...
1. **分散（Scatter）:** AgentCoreの特性上、記憶は「誰が経験したか」というアクターごとに分かれています。そのため、まずは個別にバケツを覗きに行く必要があります。
    
2. **集約（Merge）:** 取得した直後は「田原のログ」「田中のログ」という単位で、時間は前後しています。
    
3. **整列（Sort）:** ここが肝です。全ての断片を時間軸で串刺しにすることで、実際の「会話の流れ」を再現します。
    
4. **窓（Slice）:** 最後に「最新の数件」という窓を重ねることで、エージェントが「今、何の話をしているか」を認識できる最小限かつ最適なデータになります。
