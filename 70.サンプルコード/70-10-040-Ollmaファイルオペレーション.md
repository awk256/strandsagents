...
```mermaid
graph TD
    User([ユーザー]) -->|指示: PDFを要約して保存| Agent
    
    subgraph Local_PC [自分のPC内]
        Agent[Strands Agent] <--> LLM[Ollama: llama3.2]
        
        subgraph Tools [提供されたツール]
            FR[file_read: PDF/TXT対応]
            FW[file_write: ファイル書き込み]
            LD[list_directory: フォルダ一覧]
        end
...
- **Ollamaの自動制御**:
    ...
    - `subprocess`を使い、プログラム開始と同時にOllamaサーバーを立ち上げます。
        
    - `signal`を使い、Ctrl+Cで中断してもサーバーが裏で残り続けないよう配慮されています。
        
- **マルチフォーマット対応ツール**:
    
    - `file_read`: 通常のテキストだけでなく、`PyPDF2`を使ってPDFの内容も抽出できます。
        
    - `file_write`: 必要なフォルダがなければ自動作成して保存します。
        
- **タスクの連鎖（一気通貫）**:
    
    - 「PDFを読む」→「内容を要約」→「テキストファイルに保存」という複数の工程を、1つの指示でAIが順序立てて実行します。
        
- **ローカル完結型**:
    
    - `OllamaModel`を使用しているため、機密性の高いドキュメント（原子力白書など）を外部のクラウドサービスに送信せずに処理可能です。
...
- **シグナルハンドリング (`cleanup_ollama`)**:
    
    - 外部プロセス（Ollama）を扱う際の「行儀の良さ」が実装されており、PCのメモリを無駄に食い潰さない設計です。
        
- **ツール引数の型定義**:
    
    - 各ツールの引数に型ヒントと詳細なDocstringが書かれており、AIが「どのツールをどう使うべきか」を正確に理解できるようになっています。
        
- **PDF抽出ロジック**:
    
    - PDFからテキストを抽出するループ処理がツール内に組み込まれており、AIには「テキストとして」情報を渡す橋渡しをしています。
        
- **エラー処理**:
    
    - ファイルが見つからない場合などに、エラーメッセージをAIに返すことで、AIがユーザーに状況を説明できるように工夫されています。
...
try:
    ...
    local_agent(
        "`sample_file/原子力白書(令和6年度版).pdf`を読み込んで概要を400文字以内に要約し、主要なトピック10項目を箇条書きでまとめて`report.txt`に書いてください。"
    )
    print("✅ 全てのタスクが完了しました。")
...
```
**出典:**
https://github.com/strands-agents/samples/blob/main/01-tutorials/01-fundamentals/02-model-providers/01-ollama-model/ollama_file_ops_agent.ipynb


# 実行

サンプルファイルをダウンロードする。
[sample_file/原子力白書(令和6年度版).pdf](https://www.aec.go.jp/kettei/hakusho/2024/pdf/zentai.pdf)
