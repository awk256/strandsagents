...
ここでは、**「朝まで生テレビ」** を **Amazon Bedrock エージェント版**として再現・実行しています。  
Bedrock エージェントのマルチエージェント・コラボレーション機能を活用し、各エージェントがパネリストとして議論に参加する構成となっています。
...
sequenceDiagram
    ...
    participant Supervisor as Supervisor Agent<br/>(司会: 田原総一朗)
    participant Expert as Sub-Agents<br/>(専門家パネリスト)
    participant KB as Knowledge Base / Tools<br/>(共通知識・インターネット)

    Note over App, KB: 【1. 目的の共有と役割の配分】
    App->>Supervisor: 討論開始の指示 (Topic)
    Supervisor->>KB: 最新のニュース・事実を確認 (Grounding)
    KB-->>Supervisor: 客観的な情報を提供
    Note over Supervisor: 各専門家への「問い」を動的に生成

    Note over App, KB: 【2. 専門性の衝突 (Collaboration)】
    Loop 討論の深化
        Supervisor->>Expert: 専門的見解を要求 (Routing)
        Note right of Expert: 各エージェントが自身の<br/>専門プロンプトで思考
        Expert-->>Supervisor: 独自の立場からの回答 (Response)
        
        Note over Supervisor: 回答を分析し、次の矛盾点や<br/>対立軸を特定
        
        Supervisor->>Expert: 別の専門家へ反論・確認を要求
        Expert-->>Supervisor: 異なる視点からの知見
    End

    Note over App, KB: 【3. 知見の統合】
    Supervisor->>Supervisor: 全エージェントの意見を要約・集約
    Supervisor-->>App: 最終的な討論サマリーを出力
```

- **動的なオーケストレーション (Supervisorの役割)**: Python側が個別に各パネリストを叩くのではなく、Supervisor（田原総一朗）が**「今どの専門家の意見が必要か」をリアルタイムで判断**しています。これが「コラボレーション」の司令塔です。
    
- **ナレッジの共有とグラウンディング**: 各エージェントが共通のナレッジ（KB）や検索ツールにアクセスできるため、**「同じ事実に基づきつつ、異なる解釈をぶつけ合う」**という、高度な専門家会議のような動きを表現しています。
    
- **抽象から具体への変換**: Appからの「討論して」という抽象的な要求を、Supervisorが専門家向けの具体的なタスクへ分解（Decomposition）して渡すことで、**単一のAIでは到達できない多角的な回答**を生み出しています。

# Bedrockエージェント作成

[70-90-031.朝まで生テレビーBedrockエージェントによる開発＋](70-90-031.朝まで生テレビーBedrockエージェントによる開発＋.md)
