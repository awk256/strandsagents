...
flowchart LR
    subgraph Local["ローカル環境"]
        Main["debate_battle.py<br/>討論マネージャー"]
        Mod["司会者Agent<br/>（田原総一朗）"]
        Pan["パネリストAgents<br/>（6名）"]
    end
    ...
    subgraph AWS["AWS Cloud"]
        ...
        subgraph Lambda["Lambda Functions"]
            L1["search_taiwan_news"]
            L2["search_tahara_personality"]
        end
...
- ローカルからCognito（Client Credentials）でJWTトークンを取得
- トークンをAuthorizationヘッダーに付与してGatewayに接続

**ツール呼び出し**

- AgentCore GatewayがLambda関数をMCPツールとして公開
- エージェントは`search_taiwan_news`で最新情報、`search_tahara_personality`で司会者人格を取得

**エージェント推論**

- 司会者・パネリストは各々独立したAgentインスタンス
- Bedrock Claude Sonnetに直接リクエストして応答生成

**セッション管理**

- 討論状態（発言履歴、ラウンド数等）をS3にJSON保存
- Ctrl+C中断後も再開可能

**討論ループ**

- 司会者が質問 → パネリストが回答 → 次のパネリストへ
- 6名一巡でラウンド終了、無限ループ（Ctrl+Cまで継続）
...
with mcp:
    tools = mcp.list_tools_sync()  # Lambdaがツールとして見える
```

### Cognito M2M認証

```python
# Client Credentialsフローでトークン取得
...
```python
# システムプロンプトで人格を注入
agent = Agent(model=model, system_prompt=persona_prompt, callback_handler=None)
response = agent("質問テキスト")
```
...
# 発言追加
state["statements"].append({"speaker_id": sid, "content": text, ...})
```
...
def _on_interrupt(self, *_):
    self.running = False
    self._summarize()  # LLMで各参加者の発言を要約
    sys.exit(0)
```
...
AgentCoreゲートウェイに2つのLamba Function設定します。

>>[注]
>>この検証では、VPCの設定をスキップしています。

---
Amazon Bedrock AgentCore.Gatewas.Create gateway:
...
- **IAM Permissions: Create and use a new service rolle
- **Service role name: AmazonBedrockAgentCoreGatewayDefaultServiceRole1767347537681(default)
- **Service role name: `My Service Role Name`+ test
---
- **Target name 1: target-quick-start-f1f98c(default)**
- **Target Type: Lambda ARN
- **Lambda ARN: arn:aws:lambda:us-east-1:`My Account ID`:function:search_taiwan_news
- **Target schema: Define an inline schema
- **In-line scehma editor:
```
search_taiwan_news.jsonの内容をコピーペ。
```
---
- **Add another target: Click**
- **Target name 2: target-quick-start-cd3847(default)**
- **Lambda ARN: arn:aws:lambda:us-east-1:`My Account ID`:function:search_taiwan_news
- **Target schema: Define an inline schema
- **In-line scehma editor:
```
search_tahara_personality.jsonコピーペ
```
---
- **Create gateway: Click**

ゲートウェイが作成できたら、Gatewaysで`My Gateway Name`をクリックし、以下のような情報を控えます。

- **Gateway resource URL: `https://gateway-quick-start-d41057-bojslcregg.gateway.bedrock-agentcore.us-east-1.amazonaws.com/mcp`
	- **AGENTCORE_GATEWAY_ENDPOINT
- **Discovery URL: `https://cognito-idp.us-east-1.amazonaws.com/us-east-1_6EiNfrH8r/.well-known/openid-configuration`
...
- **Amazon Cognito.User pools.`User Pool Name`: Click**
>>AgentCore.Discovery URL: https://cognito-idp.us-east-1.amazonaws.com/us-east-1_6EiNfrH8r
>>の「us-east_6EiNfrH8r→User poo ID」で **User Pool Name** を特定する

  - **App Client.App client name: Click
	 - **Client ID: 3oof9hskpofsh8gpqso04st696
		 - COGNITO_CLIENT_ID
	- **Client secret: `***************************************************`
...
---
	...
	-  **Quick-setup quide.Python: On
- **③Add and configure the _authlib_ OAuth component.
	- **oauth.register→client_kwargs→scopoe: gateway-quick-start-d41057/genesis-gateway:invoke
