...
```mermaid
graph TD
    subgraph Agents ["討論参加者 (App Logic)"]
        A1[田原]
        A2[パネリスト]
    end

    subgraph Keys ["データ整理のキー (LTM/STM共通)"]
        direction TB
        MID["1.Memory ID (討論システム全体)"]
        SID["2.Session ID (第1回討論会)"]
        AID["3.Actor ID (田原 / 田中 / etc.)"]
    end

    subgraph ManagedMemory ["AgentCore Memory"]
        Store[(マネージド記憶ストア)]
    end

    %% 情報の流れ
    Agents <==>|フック経由で自動読込/保存| Keys
    Keys <==> Store
```

# 実装のポイント

- **Hookによる自動化:** `on_agent_initialized` で過去の履歴をプロンプトに自動注入し、`on_message_added` で新しい発言を自動保存します。これにより、メインの討論ロジック内に保存処理を書く必要がありません。
    
- **タグ付け保存:** `【田原】こんにちは` のように、テキスト自体に発言者名を付与して保存することで、複数人の混ざった履歴を後から読みやすくしています。
    
- **イベントの集約（_summarize）:** 最後に `list_events` を `actor_id` ごとにループで回して収集し、タイムスタンプ順にソートして全履歴を復元しています。
    
- **Gatewayとの連携:** MCPツール（`prep_agent`）を使用して、リアルタイムに外部から最新ニュースや人格情報を取得してから討論を開始しています。


[注]
AgentCore Gatewayは、[70-30-010-20.朝まで生テレビーAgentCore版](70.サンプルコード/70-30-010-20.朝まで生テレビーAgentCore版.md)を参照してください。

# プロビジョニング

### 作成開始

Amazon Bedrock AgentCore.Memory.Create memoryをクリックします。
- **Memory name: memory_masoy**
- **Short-term memory (raw event) expiration: 7 days(default 30 days)**
- **Create memory**

メモリが作成が開始されてから、MemoryIDが取得可能です。
- **MemoryId: memory_masoy-2JQxnf4Pnc**

Memory IDを環境変数に追加します。
