...
>[注]
>このコードは、実際にクリエーションライン社内で実装・検証を行った内容をベースにしています。  
本サイト用にあたっては、DBへのSQL発行処理や結果のCSVダウンロードなど一部の機能を省略していますが、プロンプト部分には一切手を加えておらず、100%そのまま公開しています。
AIがランダムに生成した310項目のSQLベンチマークに対し、99.7%の正解率（SQL実行成功）という高い成果を記録しました。
