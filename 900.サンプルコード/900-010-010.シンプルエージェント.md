...
これは最もシンプルなエージェント構成です。
ここに `tools=[]` 引数を追加して関数を渡せば、エージェントが自律的に外部APIを叩く「**Tool Use（ReAct）**」構成へと拡張できるのがStrandsの設計の特徴です。

>本書では、単なるAIモデルへのAPI呼び出しを「シンプルエージェント」と呼び、実務的な「シングルエージェント」とは明確に区別しています。Strands SDKにおいて、マルチエージェントに対するシングルエージェントと呼べる最小単位は、**「脳（LLM）」・「道具（Tool）」・「記憶（Session）」**　という3大要素を兼ね備えた存在です。
