...
sequenceDiagram
    autonumber
    actor User as ユーザー
    participant Manager as マネージャー (親)
    participant Gen as 生成エージェント ①
    participant Val as 検証エージェント ②
    participant Files as ローカル定義ファイル

    User->>Manager: 自然言語でのデータ照会依頼
    
    rect rgb(240, 245, 255)
        Note over Manager, Gen: ステップ1：コンテキストに基づくSQL生成
        Manager->>Gen: SQL生成を依頼
        Gen->>Files: ユーザー定義・スキーマを参照
        Note right of Files: user_profile.json<br/>bookshop_schema.json
        Gen-->>Manager: JSON形式でSQL+解説を返却
    end

    rect rgb(235, 255, 235)
        Note over Manager, Val: ステップ2：クエリの安全・構文チェック
        Manager->>Val: 生成されたSQLの検証を依頼
        Val->>Val: シンタックスチェック・破壊的クエリの排除
        Val-->>Manager: 修正または確認済みSQLを返却
    end

    Manager-->>User: ヒューマンフレンドリーな最終回答
```

#### ファイルベースのコンテキスト注入

**生成エージェント（Gen）**は、あらかじめ用意された `user_profile.json`（ユーザーの好みや権限）と `bookshop_schema.json`（データベースの構造）を直接参照します。これにより、AIがテーブル名やカラム名を推測で捏造する「ハルシネーション」を防ぎ、現在の環境に完全に適応した正確なクエリを作成できます。

#### 責任の分離による二重チェック体制

**マネージャー（親エージェント）**は自らSQLを書かず、あえて役割を「生成」と「検証」に分離しています。**検証エージェント（Val）**という「批判的な目」を第2ステップに置くことで、構文エラーはもちろん、不適切なデータ操作（DROPやDELETEなど）が含まれていないかを実行前にフィルタリングし、システムの安全性を担保します。

#### JSONによる構造化された内部通信

エージェント間のデータのやり取りに **JSON形式** を採用している点が重要です。SQL本体と「なぜそのクエリになったか」という解説を分けて管理することで、最終的な **標準出力（Output）** において、ユーザーに対して「技術的な正解」と「人間向けのわかりやすい説明」を同時に、かつ整理された形で提供することを可能にしています。
...
『[900-090-021.実行結果](900-090-021.実行結果.md)』
